...
import { ExtensionContext, Disposable, StatusBarItem, window, StatusBarAlignment, workspace, TextEditor, commands, Position, Selection, TextLine, TextDocument, Range } from "vscode";
        ...
        editor.edit((editBuilder) => {
            for (let i = 0; i < editor.selections.length; i++) {
                ...
                if (line.isEmptyOrWhitespace) {
                    const range = getLineDeletionRange(editor.document, line);
                    if (range) {
                        editBuilder.delete(range);
                        selections.push(new Selection(range.end, range.end));
                    } else {
